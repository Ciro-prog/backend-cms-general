...
def setup_frontend():
    ...
    #try:
     #   from .frontend.routers.admin import router as admin_frontend_router
      #  app.include_router(admin_frontend_router, tags=["admin-frontend"])
       # logger.info("✅ Router admin frontend incluido")
    #except Exception as e:
     #   logger.warning(f"⚠️ Router admin frontend no disponible: {e}")
    ...
    try:
        ...
        logger.warning(f"⚠️ Frontend router general no disponible: {e}")
        
        # Ruta básica de fallback para la raíz
        @app.get("/")
        async def home():
...
def setup_template_filters():
    ...
    logger.info("✅ Filtros de templates configurados")
...
def setup_admin_routes():
    """Configuración específica para rutas de admin"""
    try:
        ...
        if not os.path.exists(admin_templates_dir):
            ...
            logger.warning(f"⚠️ Directorio {admin_templates_dir} creado")
        ...
        if missing_templates:
            logger.warning("⚠️ Templates faltantes para admin:")
            for template in missing_templates:
                logger.warning(f"   - {template}")
        else:
            logger.info("✅ Todos los templates de admin encontrados")
            
    except Exception as e:
        logger.warning(f"⚠️ Error configurando rutas admin: {e}")
...
async def save_api_configuration(request: Request):
    ...
    try:
        ...
        # Agregar configuración de auth según el tipo
        ...
        # Aquí integrarías con tu sistema de guardado
